    ...
    BaseCharacter* attack(vector<BaseCharacter*> charList, int attackerIndex) override {
        ...
        for (vector<BaseCharacter*>::iterator it = charList.begin() + 1; it != charList.end(); ++it) {
            if ((rand() % 100 + 1) <= ((0.0236 + sqrt(0.02512 * this->getLevel() + 0.00573168))/0.01256) && (*it) != opp) {
                ...
                int r = (rand() % 3);
                if (r == 0) addAbilityMessage(this->character->getName() + " managed to also hit " + (*it)->getName() + " and deal " + to_string((*it)->getLatestDamage()) + " damage.");
                else if (r == 1) addAbilityMessage(this->character->getName() + " managed to attack " + (*it)->getName() + " and deal " + to_string((*it)->getLatestDamage()) + " damage too."); 
                else addAbilityMessage(this->character->getName() + " also hit " + (*it)->getName() + " and dealt " + to_string((*it)->getLatestDamage()) + " damage.");
