 ...
 public:
    ...
    virtual int defend(vector<BaseCharacter*> charList, int attackerIndex, int damage) {
        if (rand() % 2) {
            ...
            int r = rand() % 3;
            if (r == 0) addAbilityMessage(charList.at(attackerIndex)->getName() + " faced a wall of armor while trying to hurt " + this->character->getName());
            else if (r == 1) addAbilityMessage(charList.at(attackerIndex)->getName() + "'s damage was reduced while trying to hurt " + this->character->getName());
            else addAbilityMessage(charList.at(attackerIndex)->getName() + " faced difficulty trying to hurt " + this->character->getName());  
